 ...
 *
 * COPIA de investigacion-libro-iva-faltantes.js adaptada a la empresa 5 (GABEL BRIAN
 * OSCAR, Monotributista, puntoVta 4) - se deja el original de SUCEDE SRL (empresa 1)
 * intacto para no perderlo. Misma logica, solo cambian CUIT/punto de venta/tipo de
 * comprobante (Factura C, unico tipo que emite un Monotributista) y los huecos.
 *
 * Objetivo: para cada numero de comprobante "faltante" detectado por el control de
 * correlatividad del Libro IVA (huecos en la numeracion de esta empresa), consulta a
 ...
 *
 * Las Notas de Credito C que tambien podrian ser parte de los huecos (existen en ARCA
 * pero nunca se guardaron en el sistema) se consultarian igual que las facturas -no hay
 * ninguna en HUECOS hoy porque el control no detecto huecos de NC para esta empresa,
 * pero la logica queda armada por si aparecen en una proxima corrida.
 *
 * Requiere: certificados de PRODUCCION de esta empresa en src/certs/20426453682/{cert,key}
 * y config.pc.json con acceso de lectura a la base.
 *
 * Uso: node scripts/investigacion-libro-iva-faltantes-empresa5.js
 */
...
const CUIT_EMPRESA = 20426453682; // empresa 5, GABEL BRIAN OSCAR (Monotributista)
const PTO_VENTA = 4;
const ID_EMPRESA = 5;
...
// Empresa 5 (GABEL BRIAN OSCAR, Monotributista) - solo emite Factura C
// (PrepararFacturacionCliente fuerza Factura C para empresas Mono). Huecos
// detectados por el control de correlatividad del Libro IVA: 2-3 y 51-52.
const HUECOS = [
    { tipoFactura: 11, descripcion: 'FACTURA C', rangos: [[1, 4], [50, 53]] },
];
...
async function obtenerAfipProduccion() {
    const certFolder = path.resolve(ROOT, 'src/certs', String(CUIT_EMPRESA));
    ...
    const ticketPath = path.resolve(ROOT, 'tokens-investigacion', String(CUIT_EMPRESA));
...
async function main() {
    console.log('Instanciando AFIP (produccion, solo consulta) para empresa 5 (GABEL BRIAN OSCAR)...');
    ...
    const filas = [];
    const asociaciones = []; // CbtesAsoc leido de las NC C que tambien estaban faltantes
    for (const grupo of HUECOS) {
        ...
        for (const numero of numeros) {
            ...
            if (grupo.tipoFactura === 13 && r.CbtesAsoc?.CbteAsoc) {
    ...
    // --- 2) Adicionalmente, revisar tambien las NC C que SI estan en el sistema
    //         (por si alguna de ellas -ya conocida- cancela alguna factura faltante) ---
    console.log('\nBuscando Notas de Credito C ya registradas en el sistema para revisar CbtesAsoc...');
    const [ncConocidas] = await conn.query(
        `SELECT vf.ticket FROM ventas_factura vf INNER JOIN ventas v ON v.id = vf.idVenta
         WHERE v.idEmpresa = ? AND vf.tipoFactura = 13 AND vf.ptoVenta = ? ORDER BY vf.ticket`,
        [ID_EMPRESA, PTO_VENTA]
    );
    for (const nc of ncConocidas) {
        const info = await afip.electronicBillingService.getVoucherInfo(nc.ticket, PTO_VENTA, 13);
    ...
    console.log(`NC C (faltantes + registradas) con asociacion encontrada: ${asociaciones.length}`);

    for (const fila of filas) {
        if (fila.tipoFactura === 'NOTA DE CREDITO C') continue;
        const tipoArca = 11;
        const match = asociaciones.find(a => a.tipoAsociado === tipoArca && a.ptoVentaAsociado === PTO_VENTA && a.nroAsociado === fila.numero);
        if (match) {
            fila.observacion = `YA CANCELADA por NC C Nro ${match.ncNumero}` + (fila.observacion ? ' | ' + fila.observacion : '');
        }
    }
    ...
    const outPath = path.resolve(ROOT, 'Investigacion_Comprobantes_Faltantes_GABEL_BRIAN.xlsx');
